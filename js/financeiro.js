    ...
    if (modalLogin && window.colaboradores && window.colaboradores.length > 0) {
    ...
    function checkLogin() {
        ...
        const colab = window.colaboradores.find(c => c.senhaLogin === senha);
        
        if (!isDono) {
            const limiteBloqueio = config.horarioBloqueio || '18:30';
            // Se o horário de bloqueio for "00:00", consideramos que não há bloqueio noturno, mas se houver...
            if (horaAtualStr >= limiteBloqueio && limiteBloqueio !== "00:00") {
                window.customAlert('Acesso Negado: Fora do horário de expediente.', 'warning');
                return;
            }
        }

        if (colab) {
            window.loggedUser = colab;
            modalLogin.style.display = 'none';
            window.customAlert(`Bem-vindo, ${colab.nome}!`, 'success');
        ...
        } else if (senha === window.lojaConfig.senhaGerente) {
            window.loggedUser = { nome: 'Dono (Master)', cargo: 'Dono' };
            modalLogin.style.display = 'none';
            window.customAlert('Bem-vindo, Dono!', 'success');
        } else {
            window.customAlert('Senha incorreta!', 'warning');
        }
    }
    ...
    if (btnEntrarLogin) {
        btnEntrarLogin.addEventListener('click', checkLogin);
    }
    if (inputSenhaLogin) {
        inputSenhaLogin.addEventListener('keypress', (e) => {
            if (e.key === 'Enter') checkLogin();
        });
        
        // Dynamic Avatar preview
        inputSenhaLogin.addEventListener('input', (e) => {
            const val = e.target.value.trim();
            const match = window.colaboradores.find(c => c.senhaLogin.startsWith(val) && val.length > 2);
            if (match && match.foto) {
                ...
                avatarPreview.innerHTML = '<i class="ph ph-user" style="font-size: 40px; color: var(--text-muted);"></i>';
            }
        });
    }
