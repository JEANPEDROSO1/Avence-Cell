const sdk = require('node-appwrite');
const client = new sdk.Client()
    .setEndpoint('https://sfo.cloud.appwrite.io/v1')
    .setProject('6a959bb30013a97d1f1b')
    .setKey('standard_');
const databases = new sdk.Databases(client);

async function add() {
    try { await databases.createStringAttribute('avence_db', 'clientes', 'documento', 100, false); console.log('documento'); } catch(e){ console.error(e) }
    try { await databases.createStringAttribute('avence_db', 'clientes', 'email', 255, false); console.log('email'); } catch(e){ console.error(e) }
}
add();
